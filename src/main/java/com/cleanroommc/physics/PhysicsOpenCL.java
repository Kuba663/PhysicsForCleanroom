...
public final  class PhysicsOpenCL {
    private static CLCapabilities PLATFORM_CAPABILITIES,
            DEVICE_CAPABILITIES;

    public static void initOpenCL(MemoryStack stack) {
        CL.create();
        ...
        PhysicsCommon.LOGGER.info("Selected OpenCL platform: {}", platform.name);
    ...
    private static String getPlatformAttribute(int attribute, long platform, MemoryStack stack) {
    ...
    private record Platform(long pointer, String name, int versionMajor, int versionMinor, boolean cuda) implements Comparable<Platform> {
        ...
        @Override
        public int compareTo(PhysicsOpenCL.@NonNull Platform o) {
